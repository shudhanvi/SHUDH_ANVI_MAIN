...
//     isLoading, 
//     fetchMapDataGlobal, // Use the global fetch
//     clearMapData 
//   } = useMapContext();
...
//   // =================================================================
// //   const fetchMapData = useCallback(async (div, sec, zone) => {
...
// //     try {
// //       console.log(`🚀 Fetching Data for: ${div} -> ${sec} (Zone: ${zone})`);
// //       const zoneParam = (zone === "All") ? null : zone;
      
// //       // 1. Fetch Everything (Manholes + Wards + Buildings)
...
// //           const lat = Number(mh.latitude || mh.lat || mh.y || 0);
// //           const lon = Number(mh.longitude || mh.lon || mh.x || 0);
// //           return {
// //             ...mh,
// //             id: String(mh.manhole_id || mh.id || "unknown"), 
// //             timestamp: mh.last_operation_timestamp || mh.timestamp,
// //             zone_id: String(mh.zone_id || mh.zone || mh.sw_mh_docket_no || ""), 
// //             zone_name: mh.zone_name || mh.zone || "N/A",
// //             latitude: lat,
// //             longitude: lon,
// //             division: div,
// //             section: sec
...
// //           ...w,
// //           latitude: Number(w.latitude || w.lat || w.y),
// //           longitude: Number(w.longitude || w.lon || w.x),
// //           vertex_index: Number(w.vertex_index || w.index || 0)
// //       }));

// // // 4. ✅ Process Buildings (Fix: Unwrap the 'data' property)
// //       if (response.buildings) {
// //           console.log("🏢 Buildings received from API");
          
// //           // CHECK: Does it have a .data wrapper?
// //           if (response.buildings.data) {
// //               // ✅ Case 1: Wrapped inside .data (Your current case)
// //               setBuildingData(response.buildings.data);
// //           } else {
// //               // ⚠️ Case 2: Standard GeoJSON (Directly returned)
...
//     const invalidDivisions = ["Durgam Cheruvu", "Kondapur"];
//     dropdownOptions.forEach(item => {
//         const divName = item.division || item.sw_mh_division_no;
//         const isInvalid = invalidDivisions.some(bad => String(divName).toLowerCase().includes(bad.toLowerCase()));
//         if (divName && !isInvalid) uniqueDivisions.add(divName);
//     });
...
//     setSelectedZone("All");
//     setManholeData([]); setWardData([]); setActiveWardGeoJSON(null);setBuildingData(null);
...
//   //   setSelectedZone("All");
//   //   if (areaValue !== "All") {
//   //      fetchMapData(selectedDivision, areaValue, "All"); 
//   //   } else {
//   //      setManholeData([]); setWardData([]);setBuildingData(null);
//   //   }
//   // };
//   const handleAreaNameChange = (areaValue) => {
//     setSelectedAreaName(areaValue);
...
//       // 2. Split by comma, slash, pipe or space to handle "12345" or "1,2,3"
//       const parts = rawVal.split(/[,/| ]+/);

//       parts.forEach(p => {
//         const cleanId = p.trim();
//         if (cleanId && cleanId !== "undefined" && cleanId !== "null" && cleanId.toLowerCase() !== "na") {
//            // If the row had a specific name and only 1 ID, use it. Otherwise use the ID itself.
...
//     // 3. Return Sorted Array (Numeric Sort)
//     return Array.from(uniqueZones.entries())
//       .map(([id, name]) => ({ id, name }))
//       .sort((a,b) => {
//          const numA = parseFloat(a.id);
//          const numB = parseFloat(b.id);
//          // If both are numbers, sort numerically (1, 2, 10). Else string sort.
...
// //geojson fetching the buildings data 
 
//   // Ward Polygon Bounds
//   useEffect(() => {
//     if (!wardData.length || selectedAreaName === "All") {
//       setActiveWardGeoJSON(null); setCurrentWardBounds(null); return;
//     }
//     const validPoints = wardData.filter(w => !isNaN(Number(w.longitude)) && !isNaN(Number(w.latitude)) && Number(w.longitude) !== 0);
//     const coords = validPoints.sort((a,b) => (a.vertex_index || 0) - (b.vertex_index || 0))
//         .map(w => [Number(w.longitude), Number(w.latitude)]);

//     if (coords.length < 3) {
//       setActiveWardGeoJSON(null);
//       if (manholeData.length > 0) {
//           const bounds = new mapboxgl.LngLatBounds();
//           manholeData.forEach(m => { if (!isNaN(Number(m.longitude))) bounds.extend([Number(m.longitude), Number(m.latitude)]); });
...
//     setActiveWardGeoJSON({ type: "Feature", geometry: { type: "Polygon", coordinates: [coords] }, properties: { name: selectedAreaName } });
//     const bounds = new mapboxgl.LngLatBounds();
...
//       let displayKey = mh.zone_name;
//       if (!displayKey || displayKey === "N/A" || displayKey === "") {
...
//       acc[displayKey].push({ 
//         // ✅ FIX: Use a unique combo if ID is missing or generic
//         id: mh.id && mh.id !== "No intersecting docket found" ? mh.id : `${mh.latitude}-${mh.longitude}`, 
...
//     <div className="absolute top-full mt-1 left--4 grid grid-rows-3 gap-1 w-13.5 rounded-md overflow-hidden transform scale-y-0 opacity-0 origin-top transition-all duration-200 group-hover:scale-y-100 group-hover:opacity-100">               {mapStyles.map((style) => (
...
//               manholeGeoJSON={{ type: "FeatureCollection", features: manholeData }}
