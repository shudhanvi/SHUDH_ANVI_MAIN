...
export const RobotReportsComponent = ({ division, section, city }) => {
  ...
  /* ======================================================
     FETCH ROBOTS (ONLY IF NOT CACHED)
  ====================================================== */
  useEffect(() => {
    if (!cacheKey) return;

    // ⛔ USE CACHE
    if (robotCache.has(cacheKey)) {
      setRobots(robotCache.get(cacheKey));
      return;
    }

    let cancelled = false; // React 18 safe
    ...
    const fetchRobots = async () => {
      ...
      try {
        ...
        // 💾 CACHE
        robotCache.set(cacheKey, normalized);
  ...
  /* ======================================================
     FILTER LOGIC (MATCHES ORIGINAL BEHAVIOR)
  ====================================================== */
  useEffect(() => {
    if (!robots.length) {
      setFilteredRobots([]);
  ...
  /* -------------------- SORTING (UNCHANGED) -------------------- */
  const sortedRobots = [...filteredRobots].sort((a, b) =>
    (a.device_id || "").localeCompare(b.device_id || "", undefined, {
      numeric: true,
    })
  );
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedRobots(sortedRobots.map((r) => r.device_id));
    } else {
      setSelectedRobots([]);
    }
    setSelectAll(!selectAll);
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-lg p-[24px] border-[1.5px] border-[#E1E7EF] sm:p-6 max-h-[550px] h-[550px] relative">
        ...
        {filteredRobots.length === 0 && !isLoading ? (
          <div className="text-center text-gray-500 py-10 relative">
