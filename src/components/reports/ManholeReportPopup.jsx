...
//     : ["Top 10 Operation-Time Anomalies (Last 30 Days)"];
...
const ChartRow = ({ children }) => (
  <div className="flex flex-wrap -m-2 mb-6">{children}</div>
);
  ...
  return (
    <table className="w-full border border-gray-300 bg-white mb-4 table-fixed">
      ...
      <tbody>
        {rows.length === 0 ? (
          <tr>
            <td colSpan={finalColumns.length} className="p-2 border text-gray-600">
              No records
            </td>
          </tr>
        ) : (
            ...
            <tr key={i}>
              {finalColumns.map((c) => (
                <td key={c.key} className="p-2 border">
                  {isPlainObject(row[c.key]) || Array.isArray(row[c.key])
                    ? <pre className="text-xs">{JSON.stringify(row[c.key], null, 2)}</pre>
                    : String(row[c.key] ?? "N/A")}
                </td>
              ))}
            </tr>
          ))
...
export const ManholeReportPopup = ({ reportData, onClose }) => {
  ...
  const mapLocations = useMemo(() => {
    // SINGLE
    if (isSingle && d["Latitude"] && d["Longitude"]) {
    ...
    // MULTIPLE
    if (Array.isArray(d["Selected_Manholes"])) {
      return d["Selected_Manholes"]
        .map((m) => ({
  ...
  const cleaningFreq = Array.isArray(d["Top 5 Most Frequently Cleaned Manholes"])
    ? d["Top 5 Most Frequently Cleaned Manholes"]
    : [];

  const priorityList = Array.isArray(d["Top 5 High Priority Manholes"])
    ? d["Top 5 High Priority Manholes"]
    : [];
        ...
        <div className="flex justify-between items-center p-5 border-b">
          <h1 className="text-2xl font-bold">
            {isSingle
              ...
              : `Aggregate Report (${d["Total Manholes"]})`}
          </h1>
          <button onClick={onClose}><X /></button>
        </div>

        {/* CONTENT */}
        <div ref={printableRef} className="p-6 max-h-[85vh] overflow-y-auto bg-gray-50">
          ...
          {isSingle && (
            <>
              ...
              <div className="grid md:grid-cols-4 gap-4 mb-6">
                <InfoCard title="Manhole ID" value={d["Manhole ID"]} />
                <InfoCard title="Installation Year" value={d["Installation Year"]} />
                <InfoCard title="Latitude" value={mapLocations[0]?.lat || "N/A"} />
                <InfoCard title="Longitude" value={mapLocations[0]?.lon || "N/A"} />
              </div>

              <div className="w-full flex justify-center mb-6">
                <MiniMap locations={mapLocations} height="250px" width="50%" />
              </div>
              ...
              <ChartRow>
                {singleMonthlyFreq.length > 0 && (
                  <ChartBlock title="Monthly Operation Frequency">
                    <ChartCanvas
                      ...
                      getData={() => ({
                        labels: singleMonthlyFreq.map(([m]) => m),
                        datasets: [{ data: singleMonthlyFreq.map(([, v]) => safeNumber(v)) }],
                      })}
                      getOptions={() => ({
                        plugins: { legend: { display: false } },
                        scales: { y: { beginAtZero: true } },
                ...
                {singleMonthlyAvg.length > 0 && (
                  <ChartBlock title="Monthly Avg Operation Time (min)">
                    <ChartCanvas
                      deps={[singleMonthlyAvg, libsLoaded]}
                      getData={() => ({
                        labels: singleMonthlyAvg.map(([m]) => m),
                        datasets: [{ data: singleMonthlyAvg.map(([, v]) => safeNumber(v)) }],
                      })}
                      getOptions={() => ({
                        plugins: { legend: { display: false } },
                        scales: { y: { beginAtZero: true } },
          ...
          {isAggregate && (
            <>
              ...
              <div className="w-full flex justify-center mb-6">
                <MiniMap locations={mapLocations} height="350px" width="100%" />
              </div>
                ...
                {aggMonthlyFreq.length > 0 && (
                  <ChartBlock title="Monthly Operation Frequency">
                    <ChartCanvas
                      ...
                      getData={() => ({
                        labels: aggMonthlyFreq.map(([m]) => m),
                        datasets: [{ data: aggMonthlyFreq.map(([, v]) => safeNumber(v)) }],
                      })}
                      getOptions={() => ({
                        plugins: { legend: { display: false } },
                        scales: { y: { beginAtZero: true } },
                ...
                {aggMonthlyAvg.length > 0 && (
                  <ChartBlock title="Monthly Avg Operation Time (min)">
                    <ChartCanvas
                      ...
                      getData={() => ({
                        labels: aggMonthlyAvg.map(([m]) => m),
                        datasets: [{ data: aggMonthlyAvg.map(([, v]) => safeNumber(v)) }],
                      })}
                      getOptions={() => ({
                        plugins: { legend: { display: false } },
                        scales: { y: { beginAtZero: true } },
              ...
              <Section title="Cleaning Frequency of Selected Manholes">
                <ArrayTable
                  columns={[
                    ...
                    { key: "count", label: "Total Operations" },
                  ]}
                  rows={cleaningFreq}
                />
              </Section>
              ...
              <Section title="Operation Time Anomalies (Last 30 Days)">
                <ArrayTable rows={top10Anomalies} />
              </Section>
            </>
          )}
        </div>
