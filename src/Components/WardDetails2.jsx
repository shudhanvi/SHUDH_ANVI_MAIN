...
const WardDetails = ({ selectedWard, setSelectedWard, wardData }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, fontSize: "0.85rem", color: "#64748b" }}>
            {wardData.find((w) => w.ward_name === selectedWard)?.zone ||
              "Zone N/A"}
          </p>
        </div>
        <div
        ...
        >
          ...
          <button
            onClick={() => {
              const wardInfo = wardData.find(
                (w) => w.ward_name === selectedWard
              );
              const existingReports = JSON.parse(
                localStorage.getItem("ward_reports") || "[]"
              );
      ...
      {/* Content */}
      {wardData
        .filter((w) => w.ward_name === selectedWard)
        .map((data, idx) => {
